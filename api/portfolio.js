...
async function savePortfolio(event) {
  ...
  try {
    ...
    const htmlContent = await generatePortfolioHTML(
      portfolioData,
      templateType
    );
...
<head>
    ...
    <title>${
      data.personal?.fullName || data.personal?.name || "Portfolio"
    }</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div style="text-align: center; margin-top: 4rem; opacity: 0.7;">
            <p>Portfolio created with <a href="https://onlineportfolios.in" style="color: #ffd700;">OnlinePortfolios</a></p>
        </div>
    </div>
</body>
</html>`;
